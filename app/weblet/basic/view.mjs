...
export class MneViewContainer extends MneWeblet
{
  ...
  async findIO(frame, saveobj)
  {
    ...
    if (this.initpar.nobuttonframe)
      ...
      {
        var f = this.frame.querySelector('#buttonFrame');
        MneElement.mkClass(f, 'scroll', f.scrollHeight > f.offsetHeight);
...
export class MneView extends MneViewContainer
{
  ...
  async mkInputSPAN(id, obj)
  {
    ...
    {
      this.dpytype = MneInput.getTyp(dpytype);
      this.regexp = self.initpar.regexp[id] ?? ((MneInput.checktype[MneInput.getRegexptyp(this.dpytype)] != undefined && ( regexp == undefined || regexp.reg.toString() == MneInput.checktype.ok.reg.toString())) ? MneInput.checktype[MneInput.getRegexptyp(this.dpytype)] : regexp);
      this.format = format;
      MneElement.mkClass(obj.closest('.ele-wrapper'), "dpytype" + this.dpytype, true, "dpytype");
  ...
  async mkInputINPUT(id, obj)
  {
    ...
    obj.checkInput = () =>
    {
      ...
      if (obj.type == 'checkbox')
      {
        ...
        if (obj.getAttribute('oldvalue') == '') obj.setAttribute('oldvalue', 'false')
      }
