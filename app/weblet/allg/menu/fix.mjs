...
export class MneFixMenu extends MneMenu
{
  ...
  getReadParam(data)
  {
    ...
    {
      par.cols = this.obj.cols[data.values[0].deep] + ',' + this.initpar.showcols; 
    }
    else
    {
      par.cols = this.obj.cols[data.values[0].deep]; 
    } 

    par.scols = this.obj.cols[data.values[0].deep];

    par.wcol = data.values[0].wcol;
    par.wop  = data.values[0].wop;
    par.wval = data.values[0].wval;
  ...
  mk_submenu( container, res, data )
  {
    ...
    for ( i=0; i<res.values.length; i++)
    {
      ...
      this.mkButton(res.values[i][0], div.firstChild, { menu : div, frame : div.lastChild, values : [ values ], parent : data } , 'action');
  ...
  async values()
  {
    ...
    for ( i =0; i<this.obj.showcolnames.length; i++)
      this.obj.run.values[this.obj.showcolnames[i]] = undefined;
